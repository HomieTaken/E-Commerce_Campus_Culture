(function($){
    ...
    function createReplyComment(reply){
        var replyEl = "<div class='reply'><div><a href='javascript:void(0)' class='replyname'>"+reply.replyName+"</a>:<a href='javascript:void(0)'>@"+reply.beReplyName+"</a><span>"+reply.content+"</span></div>"
            + "<p><span>"+reply.time+"</span> <span class='reply-list-btn'></span></p></div>";
        return replyEl;
    }
    ...
    function replyClick(el){
        ...
        el.parent().parent().append("<div class='replybox'><textarea cols='80' rows='50' placeholder='来说几句吧......' class='mytextarea' ></textarea><span class='send'>发送</span><span class ='cancel'>取消回复</span></div>")
            ...
            if(content != ""){
                ...
                obj.content=content;
                obj.time = getNowDateFormat();
                var beReplyTime = parentEl.find("span:first").text();
             //   alert(beReplyTime);
                var args={content:obj.content,time:obj.time,beReplyTime:beReplyTime,type:"REPLY"};
                $.post("addComment",args,function (data) {
                    ...
                    obj.replyName = d.replyName;
                    console.log(obj.replyName);
                });
        ...
        el.parent().parent().find(".cancel").click(function(){
            $(".replybox").remove();
        });
    }
